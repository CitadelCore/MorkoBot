...
namespace MorkoBotRavenEdition.Modules
{
    ...
    internal class UserModule : MorkoModuleBase
    {
            ...
            if (!(await UserService.DoesUserHaveAnyRole(Context, ServiceProvider, "Discord Moderator")))
            {
                switch (vrole.RestrictionLevel)
                {
                    ...
                    case RoleRestrictionLevel.Unrestricted:
                        ...
                        await Context.User.SendMessageAsync(string.Empty, false, GetResponseEmbed($"Successfully added the role \"{role.Name}\" to the user.", Color.Green).Build());
                        break;
                    default:
        ...
        public async Task RemoveRoleAsync([Summary(@"The role to remove.")] IRole role, [Summary(@"The user to remove a role from (optional).")] IUser user = null)
        {
            ...
            await Context.User.SendMessageAsync(string.Empty, false, GetResponseEmbed($"Successfully removed the role \"{role.Name}\" from the user.", Color.Green).Build());
        }
